...
function buildTraditionalHtml({
  ...
  const ACCENT =
    themeConfig?.accentColor || (theme === "modern" ? "#F59E0B" : "#000000");
  const HDR_BG =
  ...
  const emptyRows = 0; // No artificial padding rows — content sizes naturally

  if (theme === "modern") {
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{font-family:${T_FONT};font-size:12px;color:${BODY_TXT};background:#fff;padding:16px;}
  ...
  <div>
    ${
      biz.bankDetails?.bankName
        ? `<div class="bank-card"><div class="lbl" style="margin-bottom:6px">Bank Details</div>
      ${[
        ["Bank", biz.bankDetails.bankName],
        ["A/C", biz.bankDetails.accountNumber],
        ...
        .map(
          ([k, v]) =>
            `<div class="bank-row"><span style="color:#6B7280">${k}</span><span style="font-weight:600">${v}</span></div>`,
        )
        .join("")}
    </div>`
        : ""
...
<table style="margin-bottom:0">
  ...
  <tbody>
    ...
    ${Array.from({ length: emptyRows })
      .map(
        () =>
          `<tr><td>&nbsp;</td><td></td>${showHsn ? "<td></td>" : ""}${showLot ? "<td></td>" : ""}<td></td><td></td><td></td><td class="r">0.00</td></tr>`,
      )
      .join("")}
  </tbody>
</table>
<table style="margin-top:-1px">
  <tr>
    <td rowspan="8" style="vertical-align:top;width:50%">
      ${
        biz.bankDetails?.bankName
          ? `<div style="font-size:9px;color:#555">Bank: ${biz.bankDetails.bankName}</div>
...
export default function InvoicePreview({
  ...
  const html = buildTraditionalHtml({
  ...
  useEffect(() => {
    ...
    // Auto-resize iframe to content height — eliminates empty whitespace at bottom
    const resize = () => {
      try {
        ...
        if (body && html) {
          ...
          setIframeHeight(Math.max(h + 8, 400));
        }
      } catch (_) {}
    };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-surface border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={() =>
              setScale((s) => (s === "desktop" ? "mobile" : "desktop"))
            }
      ...
      <div
        className={clsx(
          "flex-1 bg-stone-200 overflow-auto flex justify-center py-4",
          loading && "opacity-50",
        )}
      >
        <div
          className={clsx(
            ...
            scale === "mobile" ? "w-[375px]" : "w-full max-w-[794px]",
          )}
        >
          <iframe
            ref={iframeRef}
