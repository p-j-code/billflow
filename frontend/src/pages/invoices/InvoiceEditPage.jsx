...
const BLANK = { description: '', hsnCode: '', lot: '', qty: 1, unit: 'PCS', rate: 0, discountPct: 0, taxablePct: 5 };
...
function HsnPicker({ value, onChange }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try { const { data } = await hsnApi.search({ q, limit: 8 }); setRes(data.data.codes); } catch {}
    }, 250);
  ...
  return (
    <div className="relative">
      <input value={q} onChange={e => { setQ(e.target.value); setOpen(true); onChange(e.target.value, null); }}
        onFocus={() => setOpen(true)} placeholder="HSN" className="input-field text-xs py-1.5 px-2 font-mono" />
...
export default function InvoiceEditPage() {
  ...
  useEffect(() => {
    invoiceApi.get(id)
      .then(r => {
        ...
        setTheme(inv.pdfTheme || 'traditional');
        ...
        setItems(inv.items?.map(i => ({
          ...
          qty: i.qty || 1, unit: i.unit || 'PCS', rate: i.rate || 0,
          discountPct: i.discountPct || 0, taxablePct: i.taxablePct || 0,
        })) || [{ ...BLANK }]);
      })
  ...
  if (loading) return <div className="flex items-center justify-center h-screen"><Spinner size={24} /></div>;
      ...
      <div className={clsx('flex flex-col border-r border-border overflow-hidden transition-all', previewFull ? 'w-0' : 'w-[55%]')}>
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-surface shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(`/invoices/${id}`)} className="btn-ghost p-1.5 text-muted"><ArrowLeft size={16} /></button>
            <div>
              <h1 className="font-display font-bold text-base">Edit Invoice</h1>
              <p className="text-xs text-muted font-mono">{original?.invoiceNo}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setPreviewFull(f => !f)} className="btn-secondary text-xs py-2 flex items-center gap-1.5">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {original?.status !== 'draft' && (
            <div className="mx-4 mt-4 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2 text-xs text-amber-400">
              ⚠ Invoice is <strong>{original.status}</strong> — editing will recalculate totals. Invoice number stays the same.
            </div>
          )}
          ...
          <div className="p-4 border-b border-border space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              <FormField label="Transport"><Input value={transport} onChange={e => setTransport(e.target.value)} placeholder="Job work..." /></FormField>
              <FormField label="Vehicle No."><Input value={vehicleNo} onChange={e => setVehicleNo(e.target.value)} placeholder="MH04AB..." /></FormField>
              <FormField label="PO / Ref"><Input value={poNumber} onChange={e => setPoNumber(e.target.value)} placeholder="PO-001" /></FormField>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-0.5 bg-surface border border-border rounded-lg p-0.5">
                {[['intra','SGST+CGST'],['inter','IGST']].map(([v,l]) => (
                  <button key={v} onClick={() => setTaxType(v)}
                    className={clsx('px-3 py-1 text-xs font-semibold rounded-md transition-all', taxType===v?'bg-amber-500 text-canvas':'text-secondary')}>
                    {l}
                  </button>
                ))}
              </div>
          ...
          {partySnap && (
            <div className="p-4 border-b border-border">
              ...
              <div className="input-field flex items-center gap-3 opacity-60 cursor-not-allowed bg-surface/50">
                <div className="w-7 h-7 rounded-lg bg-amber-500/10 flex items-center justify-center text-amber-500 font-bold text-xs">{partySnap.name?.charAt(0)}</div>
                <div><p className="font-semibold text-sm">{partySnap.name}</p><p className="text-xs text-muted">{partySnap.gstin||'Unregistered'}</p></div>
              </div>
            </div>
          )}
          ...
          <div className="border-b border-border">
            ...
            <div className="divide-y divide-border">
              {items.map((item, idx) => (
                <div key={idx} className="grid items-center gap-1 px-3 py-2.5 hover:bg-surface/30 group"
                  ...
                  <HsnPicker value={item.hsnCode} onChange={(code, hsnData) => {
                    updateItem(idx,'hsnCode',code);
                    if (hsnData) updateItem(idx,'taxablePct',hsnData.gstRate);
                  }} />
                  ...
                  <p className="text-xs font-bold text-right font-mono text-primary">
                    ₹{fmt((calcedItems[idx]||item).taxableAmt||(Number(item.qty)*Number(item.rate)))}
                  </p>
            ...
            <button onClick={() => setItems(p => [...p, {...BLANK}])}
          ...
          <div className="p-4 grid grid-cols-2 gap-4 border-b border-border">
            <FormField label="Invoice Discount %">
              <Input type="number" value={invDiscPct} min={0} max={100} onChange={e => setInvDiscPct(e.target.value)} placeholder="0" />
            </FormField>
          ...
          <div className="grid grid-cols-4 gap-3 p-4 bg-surface border-t border-border">
            <div><p className="text-xs text-muted mb-0.5">Taxable</p><p className="font-mono font-bold text-sm">₹{fmt(totals.taxableValue)}</p></div>
            {taxType==='intra'
              ? <><div><p className="text-xs text-muted mb-0.5">CGST</p><p className="font-mono font-bold text-sm text-info">₹{fmt(totals.totalCgst)}</p></div>
                 <div><p className="text-xs text-muted mb-0.5">SGST</p><p className="font-mono font-bold text-sm text-info">₹{fmt(totals.totalSgst)}</p></div></>
              : <div><p className="text-xs text-muted mb-0.5">IGST</p><p className="font-mono font-bold text-sm text-amber-400">₹{fmt(totals.totalIgst)}</p></div>
            }
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-1.5">
              ...
              <p className="font-mono font-bold text-lg text-amber-400">₹{(totals.grandTotal||0).toLocaleString('en-IN')}</p>
            </div>
          </div>
        </div>
      </div>
