  ...
  try {
    ...
    const [invoices, total] = await Promise.all([
      Invoice.find(query)
        ...
        .limit(Number(limit))
        .select(
          "invoiceNo invoiceType invoiceDate dueDate status partySnapshot totals amountPaid balanceDue pdfTheme pdfThemeId createdAt",
        ),
...
const getInvoice = async (req, res, next) => {
  try {
    const invoice = await Invoice.findOne({
      ...
      .populate(
        "businessId",
        "name gstin address bankDetails invoiceSettings invoiceThemes logoUrl signatureUrl",
      );
...
const createInvoice = async (req, res, next) => {
  try {
    ...
    const partyStateCode =
      party.address?.stateCode ||
      (party.gstin ? party.gstin.substring(0, 2) : null);
    const taxType =
      overrideTaxType ||
      determineTaxType(bizStateCode, partyStateCode, party.gstin);

    // Calculate line items + totals
    const { items: calcedItems, totals } = calcInvoiceTotals(
      items,
      taxType,
      Number(invoiceDiscountPct),
    );
    ...
    const due = dueDate
      ...
      : new Date(
          invDate.getTime() +
            (business.invoiceSettings?.defaultDueDays || 30) * 86400000,
        );
    ...
    const invoice = await Invoice.create({
      ...
      pdfTheme,
      pdfThemeId: pdfThemeId || null,
      pdfThemeConfig: pdfThemeConfig || null,
      notes: notes || business.invoiceSettings?.notes,
      termsAndConditions:
        termsAndConditions || business.invoiceSettings?.termsAndConditions,
      financialYear: getFY(),
...
const updateInvoice = async (req, res, next) => {
  try {
    ...
    if (["cancelled", "void"].includes(existing.status))
      return next(
        new AppError("Cannot edit a cancelled or void invoice.", 400),
      );
...
const calculateInvoice = async (req, res, next) => {
  try {
    ...
    if (!items.length)
      return res.json({ success: true, data: { items: [], totals: {} } });
